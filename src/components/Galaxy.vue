...
// Galaxy 星河背景（Vue Bits / ogl）。GLSL ES 1.00。
// 作为深色模式全屏背景：transparent=false 得到不透明黑底+星点填满屏幕；
// 背景层 pointer-events:none 收不到鼠标事件，故 mouse-interaction 关闭。
// 注：丢掉了原版 watch(()=>props,{deep}) 重建逻辑——本应用特效靠 v-if 切换重新挂载，
// props 为静态；内联数组 prop 会在父组件重渲染时产生新引用触发无谓的 WebGL 重建。
...
const setup = () => {
  ...
  // 全屏背景定位（与 PrismaticBurst / FaultyTerminal / Aurora 一致）
